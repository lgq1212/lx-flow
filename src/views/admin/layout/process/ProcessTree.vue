...
export default {
  name: "ProcessTree",
  components: {Node, Root, Approval, Task, Cc, Trigger, Concurrent, Inclusive, Condition, Delay, Empty},
  data() {
    return {
      valid: true
    }
  ...
  methods: {
      ...
      if (Process.isPrimaryNode(node)){
        ...
        branchItems.unshift(h('div',{'class':{'add-branch-btn': true}}, [
          h('el-button', {
            ...
            props: {size: 'small', round: true, icon:"el-icon-search"},
            on:{click: () => this.addBranchNode(node)},
            domProps: {innerHTML: `添加${Process.isConcurrentNode(node)?'分支':'条件'}`},
          }, [])
        ]));
    ...
    insertNode(type, parentNode){
      ...
      switch (type){
        case 'APPROVAL': this.insertApprovalNode(parentNode); break;
        ...
        case 'INCLUSIVES': this.insertInclusivesNode(parentNode); break;
    ...
    insertDelayNode(parentNode){
      this.$set(parentNode.children, "name", "延时等待")
    ...
    addBranchNode(node){
      if (node.branchs.length < 40){
        ...
        this.$message.warning("最多只能添加 40 项😥")
      }
    },
    //删除当前节点
    delNode(node){
